...
在天玄链中，智能合约是一段完整的业务代码，其中定义了对区块链数据状态读写的规则。而交易则是用户发起对智能合约程序的调用，实现对数据状态的变更媒介，其记录了区块链中数据状态发生变化的一切。交易的执行过程反映了用户在区块链上的活动，也是区块链系统的主要工作。
...
当用户与应用客户端交互式，发生了区块链状态变更的操作，客户端需要通过天玄链 *SDK* 构造一笔带用户签名的有效交易，以在区块链中执行这些状态变更。该笔交易包含：
...
* **交易签名**：使用的代表用户身份的地址产生的签名，以便天玄链对交易进行验证，目的是防止伪造交易。
...
通常，区块链系统的运行被抽象为状态机，按照基于相同的初始状态开始，执行相同顺序的状态读写日志（即，交易），最终会到达相同的状态原则，一般会采用串行执行模型保证区块链状态的一致。在天玄链中，交易执行采用了并行执行引擎。因此，通过开发者指定的依赖构造交易的依赖图，即 *DAG* 图，再根据 *DAG* 节点依赖关系对交易进行并行执行。
