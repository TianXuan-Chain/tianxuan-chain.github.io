...
在非分片架构下，共识网络使用`异步流水线 BFT`和`完全 n 叉拓扑网络`实现**每秒 10\~30 万**的交易序列确认。解决了复杂交易执行耗时长导致的网络抖动以及单节点广播速度慢等问题。而后通过 `通用DAG状态分析器` 和 `并行执行引擎` 对已确序交易进行快速执行。最后再通过 `Verfiy_BFT 算法` 实现共识状态的快速变更。
